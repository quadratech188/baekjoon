...
public:
	...
	void safe_connect(index_t parent, index_t child, edge_t edge = {}, vertex_t const& new_vertex = {}) {
		index_t max = std::max(parent, child);
		if (max >= size())
			resize(max + 1, new_vertex);

		connect(parent, child, edge);
	}
...
int main() {
	...
	for (uint i = 0; i < n; i++) {
		...
		for (uint j = 0; j < k; j++) {
			...
			std::cin >> referrer;
			// Reverse
			graph.safe_connect(index, compress.add(referrer), {}, -1);
		}
	}
